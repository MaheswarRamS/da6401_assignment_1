...
from ann import MLP, optimizer, loss_and_grad
...
def train(args):
  ...
  # Dead_Neuron check
    ...
    wandb.log({'epoch':epoch,'Train_loss':avg_loss,'Val_loss':val_loss, 'Val_accuracy': val_acc,'Val_Precision':prec, 'Val_Recall':rec, 'Val_F1': f1, 'Grad_norm_layer1': grad_norms[0] if len(grad_norms)>0 else 0,'Grad_norm_layer2':grad_norms[1] if len(grad_norms)>1 else 0,'Total_dead_neurons': total_dead,'Layer1_activation_hist': wandb.Histogram(layer1_acts),**dead_log})
